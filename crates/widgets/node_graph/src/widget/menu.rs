...
pub(crate) struct MenuEntry<T> {
    ...
    pub(crate) kind: MenuKind<T>,
}

impl<T> MenuEntry<T> {
    pub(crate) fn action(label: impl Into<String>, action: T) -> Self {
        Self {
            label: label.into(),
            icon: None,
            checked: None,
    ...
    pub(crate) fn submenu(label: impl Into<String>, children: Vec<MenuEntry<T>>) -> Self {
        Self {
            label: label.into(),
            icon: None,
            checked: None,
    ...
    pub(crate) fn palette(items: Vec<(Color32, T)>) -> Self {
        Self {
            label: String::new(),
            icon: None,
            checked: None,
    ...
    pub(crate) fn with_shortcut(mut self, sc: Shortcut) -> Self {
    ...
    pub(crate) fn with_checkmark(mut self, checked: bool) -> Self {
...
impl<T: Clone> Menu<T> {
    ...
    fn handle_keys(&mut self, ui: &mut egui::Ui, root_id: egui::Id) -> Option<T> {
    ...
    fn show_popup(&mut self, ui: &mut egui::Ui) -> (egui::Response, Option<T>) {
...
impl<T: Clone> PopupMenu<T> {
    pub(crate) fn new(popup_id: egui::Id) -> Self {
        Self {
    ...
    pub(crate) fn open_popup(&mut self, pos: Pos2, entries: Vec<MenuEntry<T>>) {
        self.popup.set_entries(entries);
        self.popup.open(pos);
    }

    pub(crate) fn open_popup_with_title(
        &mut self,
        pos: Pos2,
    ...
    pub(crate) fn render(&mut self, ui: &mut egui::Ui, _response: &egui::Response) -> Option<T> {
        let mut result = None;
