//! Native decoder execution using the shared worker pool when requested.
...
pub(super) fn effective_workers(requested: usize, metrics: &ParallelDecoderMetrics) -> usize {
    let workers = requested.min(crate::runtime::worker_pool::shared_worker_pool().workers());
...
mod tests {
    ...
    fn unconnected(watchdog: &Watchdog, name: &str) -> InputPort {
