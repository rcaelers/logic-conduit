...
impl LogicAnalyzerViewer {
    ...
    fn layout(&self, ui: &Ui, rect: Rect) -> AnalyzerLayout {
        ...
        let (name_col_width, badge_width) = ui.ctx().fonts_mut(|fonts| {
            let name_col_width = self
                ...
                .map(|name| {
                    fonts
                        .layout_no_wrap(name, name_font.clone(), Color32::WHITE)
                        .size()
                        .x
                })
                .fold(0.0, f32::max);
    ...
    fn draw_derived_annotations(
        ...
        for annotation in visible {
            ...
            painter.rect_stroke(rect, 2.0, border, StrokeKind::Inside);
            if rect.width() >= 26.0 {
                painter.text(
                    rect.center(),
                    Align2::CENTER_CENTER,
