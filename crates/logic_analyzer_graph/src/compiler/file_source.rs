...
impl RuntimeBuilder for FileSourceBuilder {
    ...
    fn accepted_kinds(&self, socket: &Socket, _state: &Value) -> Vec<PortKind> {
        match socket.def_index {
            ...
            // `signal_processing::DeferredDslFileSource`.
    ...
    fn build(
        ...
        if resolved.kind(0).is_some() {
            ...
            return Ok(Box::new(
                signal_processing::DeferredDslFileSource::new(channels).with_name(name),
            ));
        }
        let source = signal_processing::DslFileSource::new(&state.file.value, channels)
