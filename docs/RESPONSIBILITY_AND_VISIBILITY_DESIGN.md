...
| `logic_analyzer_graph` | `node`, `node_support`, `host`, `nodes`; native feature-gated `test_support` | `node` exposes contracts implemented or submitted by graph-node features. `node_support` exposes the values and restricted `NodeBuildContext` services supplied to those implementations; it does not expose the concrete compiler context. `host` exposes `CompileCtx` result extraction together with compilation, discovery, execution, and saved-document operations consumed by application hosts. `nodes` currently owns the built-in socket catalog and registry while the crate split proceeds. The explicit test-support namespace owns fake-provider adapters used by downstream integration tests. Concrete graph-node definitions and their decoder, logic, sink, and source family modules remain private; inventory submissions make those definitions discoverable without exporting their Rust types. |
