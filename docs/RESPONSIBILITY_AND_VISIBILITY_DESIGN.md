...
| `logic_analyzer_graph` | `node`, `node_support`, `host` | `node` and `node_support` are transitional forwards to the graph API contracts. `host` exposes `CompileCtx` result extraction together with compilation, discovery, execution, and saved-document operations consumed by application hosts. |
